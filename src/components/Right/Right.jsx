...
const Right=({toggleRightWidth, liked, toggleLiked})=>{
    ...
    return (
        <Box display="flex" flexDirection="column" bgcolor="white" borderRadius="10px" margin="8px 8px 8px 0" padding="8px" overflow="auto">
            ...
            <Box display="flex" flexDirection="column" border="1px solid gray" borderRadius="10px" margin="8px 0 0 0">
                <Box display="flex" justifyContent="space-between">
                    <Typography fontSize="16px" fontWeight="bolder" margin="8px">
                        On Tour
                    </Typography>
                    <Link href="/show" sx={{textDecoration:"none"}}>
                        <Typography fontSize="16px" fontWeight="bold" margin="8px" color="black" sx={{"&:hover":{textDecoration:"underline"}}}>
                            Show all
                        </Typography>
                    </Link>
                </Box>
                <Box>
                    <Button disableRipple sx={{ height: "65px",textTransform:"none",fontWeight:"bold", borderRadius: "10px", width: "calc(100% - 16px)", margin:"0 8px 8px 8px", justifyContent: "left", background:"lightgray", "&:hover": { background: "gray", color: "white" }, "&:active": { background: "black", transition: "background .7s" }}}>
                        <Box display="flex" flexDirection="column" width="49px" height="49px" color="black" bgcolor="white" borderRadius="10px" >
                            <Typography>
                                Aug
                            </Typography>
                            <Typography>
                        ...
                        <Typography fontSize="15px" fontWeight="bold" color="black" sx={{alignItems:"center", marginLeft:"15px"}}>
                            Las Vegas
                        </Typography>
                    </Button>
                    <Button disableRipple sx={{ height: "65px",textTransform:"none",fontWeight:"bold", borderRadius: "10px", width: "calc(100% - 16px)", margin:"0 8px 8px 8px", justifyContent: "left", background:"lightgray", "&:hover": { background: "gray", color: "white" }, "&:active": { background: "black", transition: "background .7s" }}}>
            ...
            <Box display="flex" flexDirection="column" borderRadius="10px" border="1px solid gray"margin="8px 0 0 0">
                <Box display="flex" justifyContent="space-between">
                    <Typography fontSize="16px" fontWeight="bolder" margin="8px" >
                        Next in queue
                    </Typography>
                ...
                <Box>
                    <Button disableRipple onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} sx={{ height: '65px', textTransform: 'none', fontWeight: 'bold', width:"calc(100% - 16px)", borderRadius: '10px',  margin: '8px', justifyContent: 'left', background:"lightgray",'&:hover': { background: 'gray', color: 'white' }, '&:active': { background: 'black', transition: 'background .7s' }}} >
                        {!isHovered && (
                            <HiMusicNote size="20px" color="black" style={{ marginRight: '8px' }} />
                        )}
                        {isHovered && (
                            <HiChevronRight size="20px" color="black" style={{ marginRight: '8px' }} />
                        )}
